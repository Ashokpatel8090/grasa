...
// import toast, { Toaster } from "react-hot-toast"; // <-- 1. Imported toast and Toaster
...
//       {/* --- 3. Added the Toaster Component here with identical styling --- */}
//       <Toaster 
//         position="bottom-right"
//         toastOptions={{ 
//           duration: 3500, 
//           style: { background: "#1b1b1b", color: "#fff", borderRadius: 8, fontSize: 13 } 
//         }} 
//       />
...
//   // Dynamic: keyed by question id (e.g. "q1", "q2", ...)
//   // For "input" type questions, store { age, gender } or whatever fields exist
...
//   // Derived
//   const TOTAL_Q = quizMeta?.total_questions ?? questionsData.length;
//   const currentQNum = currentScreen.startsWith('q')
//     ? parseInt(currentScreen.replace('q', ''))
//     : 0;
...
//           // Initialise multi-select sets for every multi_select question dynamically
...
//           // Initialise loading steps array to match question count (or a fixed set of labels)
//           setActiveSteps(new Array(data.quiz.total_questions ?? data.questions.length).fill(false));
//         }
//         setIsLoading(false);
//       })
//       .catch(err => {
...
//     // Check for an existing unexpired result
//     const saved = localStorage.getItem('grasa_longevity_result');
//     if (saved) {
//       try {
//         const parsed: StoredResult = JSON.parse(saved);
...
//     if (q.type === 'input') {
//       // Validate all fields in this question
...
//       const newSet = new Set(prev[qId]);
//       if (idx === 0) {
//         // "None" option — clears everything else
//         newSet.clear();
//         newSet.add(0);
//       } else {
//         newSet.delete(0);
...
//       }
//       let totalScore = 0;
//       newSet.forEach(i => { totalScore += optScores[i] || 0; });
//       setScores(s => ({ ...s, [qId]: totalScore }));
//       return { ...prev, [qId]: newSet };
//     });
//   };
...
//     setInputValues(prev => ({
//       ...prev,
//       [qId]: { ...(prev[qId] || {}), [fieldId]: value }
//     }));
//   };

//   // ─── Build dynamic payload ────────────────────────────────────────────────────
//   /**
//    * Builds a payload object shaped to what your API expects.
//    *
//    * Convention used (mirrors your original hardcoded payload):
//    *   - input question   → flat fields from inputValues[qId]  (e.g. q1_age, q1_gender)
//    *   - single_select    → q{n}_answer_index
//    *   - multi_select     → q{n}_selected_indices  (array)
//    *
//    * If your backend uses different keys, adjust the mapping below.
//    */
//   const buildPayload = (): Record<string, any> => {
//     const payload: Record<string, any> = { whatsapp_number: "" };

//     questionsData.forEach(q => {
//       const prefix = q.id; // e.g. "q1", "q2" …

//       if (q.type === 'input') {
//         const vals = inputValues[q.id] || {};
//         q.fields?.forEach(field => {
//           const key = `${prefix}_${field.id}`; // e.g. q1_age, q1_gender
//           payload[key] = vals[field.id] ?? (field.type === 'number' ? 0 : '');
//         });
//       } else if (q.type === 'single_select') {
//         payload[`${prefix}_answer_index`] = singleSelections[q.id] ?? 0;
//       } else if (q.type === 'multi_select') {
//         payload[`${prefix}_selected_indices`] = Array.from(multiSelections[q.id] || []);
//       }
//     });
...
//     // Reset animation state dynamically
//     const stepCount = TOTAL_Q;
//     setActiveSteps(new Array(stepCount).fill(false));
//     setCalcProgress(0);
...
//     const pct = TOTAL_Q > 0 ? Math.round(((currentQNum - 1) / TOTAL_Q) * 100) : 0;

//     return (
//       <div className="sticky top-0 z-40 border-b border-white/5 px-4 md:px-8 py-4 bg-[#0f1114]/90 backdrop-blur-md">
//         <div className="max-w-[620px] mx-auto flex justify-between items-center mb-2">
//           <span className="text-xs font-bold tracking-wider text-gray-400 uppercase">
//             Question {currentQNum} of {TOTAL_Q}
//           </span>
//           <span className="text-xs font-bold text-[#C5D82D]">{pct}%</span>
//         </div>
//         <div className="max-w-[620px] mx-auto h-[4px] bg-white/10 rounded-full overflow-hidden">
...
//   // ─── Option button ────────────────────────────────────────────────────────────
//   const OptionBtn = ({ idx, icon, label, sub, isSelected, onClick, isMulti = false }: any) => (
//     <button
//       onClick={onClick}
//       className={`group flex items-center gap-4 px-5 py-4 bg-black/40 backdrop-blur-sm border-[1.5px] rounded-xl cursor-pointer transition-all duration-200 text-left w-full h-full
//         ${isSelected
...
//         {/* INPUT type — render each field dynamically */}
//         {q.type === 'input' && q.fields && (
//           <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4 mb-6">
//             {q.fields.map(field => (
//               <div key={field.id}>
...
//                     onChange={e => setInputField(q.id, field.id, parseInt(e.target.value) || null)}
//                     className="w-full py-4 px-5 bg-black/40 border-[1.5px] border-white/10 text-white text-base rounded-xl outline-none transition-all duration-200 focus:border-[#C5D82D] focus:shadow-[0_0_15px_rgba(197,216,45,0.1)] placeholder-gray-600"
//                   />
//                 ) : (
//                   /* select / text fields */
...
//                 isSelected={
//                   isMulti
//                     ? multiSelections[q.id]?.has(opt.index)
//                     : singleSelections[q.id] === opt.index
//                 }
//                 onClick={() => {
//                   if (isMulti) {
//                     toggleMultiOpt(q.id, opt.index, q.options!.map(o => o.score));
...
//               disabled={isNextDisabled}
//               onClick={() => goTo(q.order + 1)}
//               className="w-full sm:w-64 py-4 px-8 bg-[#C5D82D] text-black font-bold text-sm rounded-full transition-all duration-200 flex items-center justify-center gap-3 disabled:opacity-30 disabled:cursor-not-allowed hover:enabled:bg-[#d9ed3d] hover:enabled:-translate-y-1 hover:enabled:shadow-[0_10px_30px_rgba(197,216,45,0.2)] uppercase tracking-widest"
//             >
//               Next <span>→</span>
//             </button>
//           ) : (
...
//   // ─── Calc screen step labels (dynamic, falls back to generic labels) ──────────
//   const calcStepLabels = questionsData.length > 0
//     ? questionsData.map(q => q.category || `Analysing question ${q.order}`)
//     : [
//         "Energy & recovery patterns", "Digestive health signals",
//         "Visceral fat & body composition", "Metabolic markers from reports",
//         "Indian genetic risk profile", "Dietary pattern analysis"
//       ];
...
//         ref={componentRef}
//         className="bg-[#0f1114] text-white w-full min-h-[100dvh] overflow-x-hidden relative leading-relaxed"
//       >
//         <div className="fixed top-0 right-0 w-[500px] h-[500px] bg-lime-500/5 blur-[120px] rounded-full -mr-48 -mt-48 pointer-events-none z-0" />
//         <div className="fixed bottom-0 left-0 w-[300px] h-[300px] bg-lime-500/5 blur-[100px] rounded-full -ml-24 -mb-24 pointer-events-none z-0" />
...
//                   <circle cx="45" cy="45" r="40" fill="none" stroke="rgba(255,255,255,0.1)" strokeWidth="6" />
//                   <circle
//                     cx="45" cy="45" r="40" fill="none" stroke="#C5D82D" strokeWidth="6"
//                     strokeDasharray="251"
//                     strokeDashoffset={251 - (251 * calcProgress / 100)}
//                     strokeLinecap="round"
//                     className="transition-[stroke-dashoffset] duration-500 ease"
//                   />
//                 </svg>
...
//               {/* Dynamic step list */}
//               <div className="flex flex-col gap-4 mt-6 mx-auto max-w-[320px] text-left">
//                 {calcStepLabels.slice(0, TOTAL_Q).map((label, i) => (
//                   <div
//                     key={i}
//                     className={`flex items-center gap-4 text-sm font-bold transition-all duration-300 ${
//                       activeSteps[i] ? 'opacity-100 text-gray-200' : 'opacity-20 text-gray-600'
//                     }`}
//                   >
//                     <div
//                       className={`w-2.5 h-2.5 rounded-full shrink-0 transition-all duration-300 ${
//                         activeSteps[i] ? 'bg-[#C5D82D] shadow-[0_0_10px_rgba(197,216,45,0.6)]' : 'bg-gray-800'
//                       }`}
//                     />
//                     {label}
//                   </div>
...
import { useRouter } from 'next/navigation';
import {
 Question, QuizMeta, ApiResultData, StoredResult
} from '@/types/longevity';
import { BASE_URL } from "@/components/config/api";
import toast, { Toaster } from "react-hot-toast";
        ...
        <div className="max-w-5xl mx-auto px-6 pt-10 pb-24 md:px-8 md:pt-14 relative z-10">
          ...
          {currentScreen === "wa_capture" && (
            <div className="animate-fadeUp relative z-20">
              ...
              <div className="text-base sm:text-lg text-gray-400 mb-8 leading-relaxed max-w-2xl">
                Where should we send your complete metabolic profile? Enter your WhatsApp number to securely lock in your answers and generate your personalised results.
              </div>
