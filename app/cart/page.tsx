...
//               ? "https://schema.org/InStock"
//               : "https://schema.org/OutOfStock",
//           seller: { "@type": "Organization", name: "GRASA Super Foods & Beverages" },
//         },
//         description: `Quantity in Cart: ${item.quantity}`,
//       },
//     })),
...
//       if (token) {
//         try {
//           const res = await fetch(`${BASE_URL}/grasa/shop/cart`, {
//             headers: { Authorization: `Bearer ${token}` }
//           })
//           const data = await res.json()
//           if (data?.cart?.id) setCartId(data.cart.id)
//           if (data?.available_coupons) setAvailableCoupons(data.available_coupons)
//         } catch (err) {
...
//   useEffect(() => {
//     const q: Record<number, number> = {}
//     cart.forEach((item) => { q[item.product_id] = item.quantity })
//     setLocalQuantities(q)
//   }, [cart])

//   const totalPrice = cart.reduce(
//     (acc, item) => acc + Number(item.effective_price) * item.quantity, 0
//   )
//   const totalItems = cart.reduce((acc, item) => acc + item.quantity, 0)

//   const finalPrice = couponResult?.final_price ?? totalPrice
...
//     const token = getCookie("token")
//     if (!token) { toast.error("Please login to apply coupons"); return }
...
//     try {
//       const res = await fetch(`${BASE_URL}/grasa/shop/coupons/validate`, {
//         method: "POST",
//         headers: {
//           "Content-Type": "application/json",
//           Authorization: `Bearer ${token}`
//         },
//         body: JSON.stringify({ coupon_code: code.trim().toUpperCase(), cart_id: cartId })
//       })

//       if (!res.ok) {
//         const errData = await res.json().catch(() => null)
//         const msg = errData?.message || errData?.detail || `${res.status} Error: ${res.statusText.toUpperCase()}`
//         setCouponError(msg)
...
//           setCouponError(null)
//           toast.success(`Coupon "${code.toUpperCase()}" applied! You save ₹${data.discount}`)
//         } else {
...
//       }
//     } catch (err) {
//       const msg = "Failed to validate coupon. Try again."
//       setCouponError(msg)
//       toast.error(msg)
//     } finally {
...
//                 <div style={{ background: "#fff", border: "1px solid #e0e0e0", borderRadius: 4, overflow: "hidden", animation: "fadeIn 0.2s ease" }}>
//                   <div style={{ background: "#f5f5f5", padding: "12px 16px", borderBottom: "1px solid #e0e0e0" }}>
//                     <span style={{ fontSize: 12, fontWeight: 700, color: "#888", textTransform: "uppercase", letterSpacing: 0.8 }}>
//                       My Cart ({totalItems})
//                     </span>
...
//                     {/* ── Integrated Coupon Form & Applied State ── */}
//                     <div style={{ marginBottom: 16 }}>
//                       {appliedCoupon ? (
//                         <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 13, color: "#555", background: "#f0fbe7", padding: "10px 12px", borderRadius: 6, border: "1px dashed #b7e087" }}>
//                           <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
//                             <CheckCircle2 size={16} style={{ color: "#3d7a1a" }} />
//                             <div>
//                               <div style={{ fontWeight: 700, color: "#1b1b1b" }}>{appliedCoupon}</div>
//                               <button onClick={removeCoupon} style={{ background: "none", border: "none", color: "#e53935", fontSize: 11, fontWeight: 600, cursor: "pointer", padding: 0, marginTop: 2 }}>Remove</button>
//                             </div>
//                           </div>
//                           <span style={{ fontWeight: 600, color: "#3d7a1a" }}>− ₹{discount.toFixed(2)}</span>
//                         </div>
//                       ) : (
//                         <div>
...
//                                 border: `1px solid ${couponError ? "#e53935" : "#e0e0e0"}`, borderRadius: 4,
//                                 outline: "none", textTransform: "uppercase", color: "#1b1b1b", background: "#fafafa"
//                               }}
...
//                               style={{
//                                 background: couponInput.trim() ? "#1b1b1b" : "#e8e8e0",
//                                 color: couponInput.trim() ? "#C5D82D" : "#888",
//                                 border: "none", borderRadius: 4, padding: "0 16px",
//                                 fontSize: 13, fontWeight: 700, cursor: couponInput.trim() ? "pointer" : "not-allowed",
//                                 display: "flex", alignItems: "center", justifyContent: "center", minWidth: 70
//                               }}
//                             >
...
//                           {couponError && (
//                             <div style={{ fontSize: 12, color: "#e53935", marginBottom: 10, display: "flex", alignItems: "center", gap: 4 }}>
//                               <X size={12} /> {couponError}
//                             </div>
//                           )}

//                           {/* Horizontal Scrolling Available Coupons */}
//                           {availableCoupons.length > 0 && (
//                             <div className="hide-scrollbar" style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 4 }}>
//                               {availableCoupons.slice(0, showAllCoupons ? availableCoupons.length : 2).map((c) => (
//                                 <div
//                                   key={c.id}
//                                   onClick={() => selectCoupon(c.coupon_code)}
//                                   style={{
//                                     flex: "0 0 140px", border: "1px dashed #C5D82D", borderRadius: 6, padding: "10px",
//                                     background: "#fdfff5", cursor: "pointer", transition: "all 0.2s", display: "flex", flexDirection: "column"
//                                   }}
//                                 >
//                                   <div style={{ fontSize: 13, fontWeight: 700, color: "#1b1b1b", marginBottom: 4 }}>{c.coupon_code}</div>
//                                   <div style={{ fontSize: 11, color: "#3d7a1a", fontWeight: 600, marginBottom: 8, flex: 1 }}>
//                                     {c.discount_type === "FixedAmount" ? `Flat ₹${parseFloat(c.discount_value).toFixed(0)} OFF` : `${c.discount_value}% OFF`}
//                                   </div>
//                                   <div style={{ fontSize: 11, color: "#1b1b1b", fontWeight: 700, background: "#C5D82D", alignSelf: "flex-start", padding: "2px 8px", borderRadius: 3 }}>
//                                     Apply
//                                   </div>
//                                 </div>
//                               ))}
...
//                                   style={{
//                                     flex: "0 0 80px", border: "1px solid #e0e0e0", borderRadius: 6, display: "flex", flexDirection: "column",
//                                     alignItems: "center", justifyContent: "center", background: "#fafafa", cursor: "pointer", color: "#555"
//                                   }}
//                                 >
//                                   <span style={{ fontSize: 14, fontWeight: 700 }}>+{availableCoupons.length - 2}</span>
...
//                           <span style={{ fontSize: 12, color: "#888", fontWeight: 400, textDecoration: "line-through" }}>₹{totalPrice.toFixed(2)}</span>
//                         )}
//                         <span>₹{finalPrice.toFixed(2)}</span>
//                       </div>
//                     </div>

//                     <button onClick={() => router.push("/checkout")} style={primaryBtn}>
//                       Proceed to Checkout
//                     </button>
//                   </div>
//                 </div>
