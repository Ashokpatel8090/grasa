...
//                 <XCircle size={38} className="text-red-500" />
//               </div>
//               <h3 className="text-2xl font-black text-[#1b1b1b] mb-1">Payment Failed</h3>
//               <p className="text-[#5c5c5c] text-sm mb-6">
//                 {paymentState === "cancelled"
//                   ? "You closed the payment window. Your subscription was not activated."
//                   : "Something went wrong while processing your payment. Please try again."}
...
//               <div className="flex gap-3">
//                 <button onClick={onClose} className="flex-1 py-3.5 rounded-lg font-bold text-[#5c5c5c] text-sm bg-[#ebecdf] hover:bg-[#e0e0d8] transition-colors border border-[#d6d1c4]">
//                   Close
//                 </button>
//                 <button onClick={onConfirm} className="flex-1 py-3.5 rounded-lg font-bold text-[#1b1b1b] text-sm bg-[#C5D82D] hover:bg-[#d4e840] transition-colors shadow-sm flex items-center justify-center gap-2">
//                   Try Again <ArrowRight size={15} />
//                 </button>
//               </div>
//             </div>
...
// /* ------------------- DETAILS MODAL ------------------- */
// function PlanDetailsModal({ plan, onClose, onProceedToPayment }) {
//   useEffect(() => {
//     document.body.style.overflow = "hidden";
//     return () => { document.body.style.overflow = ""; };
//   }, []);
...
//             </button>
//             <button
//               onClick={() => { onClose(); onProceedToPayment(plan); }}
//               className="flex-1 py-3.5 rounded-lg font-bold text-[#1b1b1b] text-sm bg-[#C5D82D] hover:bg-[#d4e840] transition-colors shadow-sm flex items-center justify-center gap-2 group"
//             >
//               Proceed to Payment
//               <ChevronRight size={16} className="transition-transform group-hover:translate-x-1" />
...
//     0%   { box-shadow: 0 0 0 0 rgba(197,216,45,0.5); }
//     50%  { box-shadow: 0 0 0 12px rgba(197,216,45,0); }
//     100% { box-shadow: 0 0 0 0 rgba(197,216,45,0); }
//   }
//   .plan-highlight-pulse {
//     animation: highlightPulse 1.2s ease-out 0.4s 2;
//   }
// `;
...
//   // For highlight-from-longevity-result feature
...
//         // ── HIGHLIGHT LOGIC ──
//         // Check URL query param ?highlight=<programme name>
...
//           // Fuzzy match: try full title match first, then partial word overlap
//           const findMatch = (plans) => {
//             // 1. Exact title match
//             let match = plans.find(p => p.title.toLowerCase().trim() === decoded);
//             if (match) return match;

//             // 2. Title is contained in the programme name or vice versa
//             match = plans.find(p =>
//               decoded.includes(p.title.toLowerCase().trim()) ||
//               p.title.toLowerCase().trim().includes(decoded)
//             );
//             if (match) return match;

//             // 3. Subtitle match
//             match = plans.find(p =>
//               decoded.includes(p.subtitle.toLowerCase().trim()) ||
//               p.subtitle.toLowerCase().trim().includes(decoded)
//             );
//             if (match) return match;

//             // 4. Word overlap score — pick highest scoring plan
//             const score = (plan) => {
//               const planWords = (plan.title + " " + plan.subtitle).toLowerCase().split(/\s+/);
//               const queryWords = decoded.split(/\s+/);
//               return queryWords.filter(w => planWords.some(pw => pw.includes(w) || w.includes(pw))).length;
//             };
//             const scored = plans.map(p => ({ p, s: score(p) })).sort((a, b) => b.s - a.s);
//             return scored[0]?.s > 0 ? scored[0].p : null;
//           };

//           const matchedPlan = findMatch(mappedPlans);

//           if (matchedPlan) {
//             const matchId = String(matchedPlan.id);
//             setHighlightedPlanId(matchId);

//             // Scroll to the matched plan after a short delay (let DOM paint)
//             setTimeout(() => {
//               const el = planRefs.current[matchId];
//               if (el) {
//                 el.scrollIntoView({ behavior: "smooth", block: "center" });
//               }
//               // Clean the URL so a page refresh doesn't re-trigger highlight
//               window.history.replaceState({}, "", window.location.pathname + "#regimen-plans");
//             }, 600);
//           }
//         }
//         // ── END HIGHLIGHT LOGIC ──
...
//             {plans.map((plan, i) => {
//               const isCurrentActivePlan = activePlanId === String(plan.id);
//               const isHighlighted = highlightedPlanId === String(plan.id);
//               const shouldDisableAllButtons = hasActiveSubscription;

//               return (
//                 <div
//                   key={plan.id || i}
//                   ref={(el) => { planRefs.current[String(plan.id)] = el; }}
//                   className={`group relative rounded-2xl flex flex-col overflow-hidden transition-all duration-500 bg-[#f4f4f2] ${
//                     isHighlighted
//                       ? "border-2 border-[#C5D82D] shadow-[0_10px_40px_rgba(197,216,45,0.35)] scale-[1.03] plan-highlight-pulse"
//                       : isCurrentActivePlan
//                       ? "border-2 border-[#C5D82D] shadow-[0_10px_30px_rgba(197,216,45,0.25)] scale-[1.03] before:absolute before:inset-0 before:rounded-2xl before:bg-gradient-to-br before:from-[#C5D82D]/20 before:to-transparent before:opacity-100 before:pointer-events-none"
//                       : "border border-[#d6d1c4] hover:border-[#C5D82D] hover:shadow-[0_12px_30px_rgba(0,0,0,0.08)] hover:-translate-y-1"
//                   }`}
//                 >
//                   {/* Badges */}
//                   {isHighlighted && !isCurrentActivePlan && (
//                     <div className="absolute top-4 right-4 z-10 bg-[#C5D82D] text-[#1b1b1b] text-[10px] font-bold px-3 py-1.5 rounded-full uppercase tracking-wider flex items-center gap-1">
//                       <span>★</span> Recommended for you
//                     </div>
//                   )}
//                   {isCurrentActivePlan && !isHighlighted && (
//                     <div className="absolute top-4 right-4 z-10 bg-[#C5D82D] text-[#1b1b1b] text-[10px] font-bold px-3 py-1.5 rounded-full uppercase tracking-wider">
//                       ✓ Active Plan
//                     </div>
//                   )}
...
//                       <button
//                         onClick={() => handleViewDetails(plan)}
//                         className="text-[12px] font-bold text-[#84940e] hover:text-[#b0c228] transition-colors mb-5 flex items-center gap-1.5 group/more"
//                       >
//                         + {plan.features.length - 2} more features
//                         <ChevronRight size={13} className="transition-transform group-hover/more:translate-x-0.5" />
//                       </button>
//                     )}
...
//                         <button
//                           onClick={() => handleOpenOverlay(plan)}
//                           className={`px-5 py-3 rounded-lg font-bold text-sm transition-all duration-200 shadow-sm whitespace-nowrap border ${
//                             isHighlighted
//                               ? "bg-[#1b1b1b] text-[#C5D82D] border-[#1b1b1b] hover:bg-[#2a2a2a]"
//                               : "bg-[#C5D82D] text-[#1b1b1b] hover:bg-[#d4e840] border-[#C5D82D]"
//                           }`}
//                         >
//                           {isHighlighted ? "Start This Plan" : plan.button}
//                         </button>
//                       )}
//                     </div>
//                   </div>
...
//           plan={detailedPlan}
//           onClose={() => setDetailedPlan(null)}
//           onProceedToPayment={handleOpenOverlay}
//         />
//       )}
//     </>
//   );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="p-8 sm:p-10">
          <div className="mb-8">
            <p className="text-[11px] tracking-[1px] font-bold text-[#5c5c5c] mb-2">{plan.weeks}</p>
            <h2 className="text-4xl  font-bold text-[#1b1b1b] mb-2">{plan.title}</h2>
            <p className="text-lg text-[#5c5c5c] font-semibold">{plan.subtitle}</p>
            <div className="w-12 h-[3px] bg-[#1b1b1b] mt-4" />
          </div>
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {plan.features.map((feature, index) => (
                <div key={index} className="flex items-start gap-3 p-2 bg-white rounded-lg border border-[#d6d1c4]">
                  <div className="mt-1 rounded-full p-0.5 flex-shrink-0 shadow-sm bg-[#C5D82D]">
                    <Check size={13} className="text-[#1b1b1b]" />
                  </div>
                  <span className="text-[14px] font-medium text-[#1b1b1b]">{feature}</span>
            ...
            {plans.map((plan, i) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="p-6 sm:p-7 flex-grow flex flex-col">
                    <p className="text-[11px]  font-bold mb-1 text-[#6a7706]">{plan.weeks}</p>
                    <h3 className="text-2xl font-bold mb-1 text-[#1b1b1b]">{plan.title}</h3>
                    <p className="font-semibold text-sm mb-3 text-[#5c5c5c]">{plan.subtitle}</p>
                    <div className="w-8 h-[2px] mb-4 bg-[#d6d1c4]" />
                    {/* <p className="text-[14px] leading-relaxed mb-5 italic min-h-[56px] text-[#5c5c5c]">{plan.description}</p> */}
                    ...
                    {plan.features.length > 2 && (
                      <button
                        onClick={() => handleViewDetails(plan)}
                        className="text-[12px] font-bold text-[#6a7706] hover:text-[#6b7805] transition-colors  flex items-center gap-1.5 group/more"
                      >
                        + {plan.features.length - 2} more features
                        <ChevronRight size={13} className="transition-transform group-hover/more:translate-x-0.5" />
                      </button>
                  ...
                  <div className="px-5 sm:px-6 pb-4 pt-0 mt-auto">
  <div className="border-t border-[#d6d1c4] pt-2 flex flex-col gap-4">

    {/* PRICE */}
    <div className="flex items-baseline justify-between">
      <div className="flex flex-col">
        {plan.oldPrice !== plan.price && (
          <span className="line-through text-xs font-semibold text-[#7a7a7a]">
            ₹{plan.oldPrice}
          </span>
        )}
        <span className="text-[22px]  font-extrabold text-[#1b1b1b] leading-tight">
          ₹{plan.price}
        </span>
      </div>
    </div>
    ...
    {!isCurrentActivePlan && !shouldDisableAllButtons && (
      <button
        onClick={() => handleOpenOverlay(plan)}
        className={`w-full py-3.5 rounded-xl font-semibold text-sm transition-all duration-200 shadow-sm ${
          isHighlighted
            ?"bg-[#C5D82D] text-[#1b1b1b] hover:bg-[#d4e840]"
            //  "bg-[#1b1b1b] text-[#C5D82D] hover:bg-[#2a2a2a]"
            : "bg-[#C5D82D] text-[#1b1b1b] hover:bg-[#d4e840]"
        }`}
      >
        {isHighlighted ? "Start Here" : plan.button}
      </button>
    )}
